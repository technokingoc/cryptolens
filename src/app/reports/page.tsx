  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 md:ml-60 pt-16 md:pt-0 p-4 md:p-8 pb-24 md:pb-8 max-w-5xl">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">📄 Analysis Reports</h1>
        <p className="text-gray-400 text-sm mb-6">Market reports and analysis from Wen</p>

        {reports.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
            <p className="text-gray-400 text-lg mb-1">No reports yet</p>
            <p className="text-gray-300 text-sm">Wen saves reports during analysis cycles</p>
          </div>
        ) : (
            ...
            {reports.map((r) => (
              <details key={r.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden group">
                ...
                <div className="px-5 py-4 border-t border-gray-100">
                  <pre className="whitespace-pre-wrap text-sm text-gray-600 font-sans leading-relaxed">{r.content}</pre>
                </div>
              </details>
            ))}
          </div>
