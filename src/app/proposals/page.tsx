  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 md:ml-60 pt-16 md:pt-0 p-4 md:p-8 max-w-5xl">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">⚡ Trade Proposals</h1>
        <p className="text-gray-400 text-sm mb-6">Wen&apos;s confluence-scored recommendations. Review and decide.</p>

        {pending.length > 0 && (
          <div className="mb-8">
            <h2 className="text-sm font-semibold text-amber-600 mb-4">🔔 Pending ({pending.length})</h2>
            <div className="space-y-5">{pending.map((p) => <ProposalCard key={p.id} proposal={p} showActions />)}</div>
          </div>
        )}

        {pending.length === 0 && (
          <div className="bg-white border border-gray-200 rounded-xl p-10 text-center mb-8">
            <p className="text-gray-400 text-lg mb-1">No pending proposals</p>
            <p className="text-gray-300 text-sm">Wen submits proposals when confluence &gt; +0.5</p>
          </div>
        )}
        ...
        {history.length > 0 && (
          <div>
            ...
            <div className="space-y-4">{history.map((p) => <ProposalCard key={p.id} proposal={p} showActions={false} />)}</div>
          </div>
        )}
      </main>
    </div>
    ...
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="text-right">
          ...
          <p className="text-[11px] text-gray-400">Confluence</p>
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <Chip label="Action" value={p.action} className={p.action === "BUY" ? "text-emerald-600" : "text-red-500"} />
          <Chip label="Bucket" value={p.bucket} />
          {p.timeHorizon && <Chip label="Horizon" value={p.timeHorizon} />}
        ...
        {/* 7 Pillars */}
        <div className="mb-4">
          <p className="text-xs text-gray-400 mb-2 font-medium">📊 Confluence Breakdown</p>
        ...
        {(p.entryPrice || p.stopLoss || p.target1) && (
          <div className="mb-4">
            <p className="text-xs text-gray-400 mb-2 font-medium">📐 Execution Plan</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {p.entryPrice && <PlanItem label="Entry" value={`$${parseFloat(p.entryPrice).toLocaleString()}`} />}
              {p.stopLoss && <PlanItem label="Stop-Loss" value={`$${parseFloat(p.stopLoss).toLocaleString()}`} className="text-red-500" />}
              {p.target1 && <PlanItem label="Target 1" value={`$${parseFloat(p.target1).toLocaleString()}`} className="text-emerald-600" />}
              {p.target2 && <PlanItem label="Target 2" value={`$${parseFloat(p.target2).toLocaleString()}`} className="text-emerald-600" />}
              {p.maxLoss && <PlanItem label="Max Loss" value={`$${parseFloat(p.maxLoss).toFixed(2)}`} className="text-red-500" />}
              {p.expectedGain && <PlanItem label="Exp. Gain" value={`$${parseFloat(p.expectedGain).toFixed(2)}`} className="text-emerald-600" />}
              {p.positionSizePct && <PlanItem label="Position %" value={`${parseFloat(p.positionSizePct).toFixed(1)}%`} />}
            </div>
          </div>
        )}
        ...
        {p.risks?.length > 0 && (
          <div className="mb-4">
            <p className="text-xs text-gray-400 mb-2 font-medium">⚠️ Risks</p>
            <ul className="space-y-1">
              {p.risks.map((r: string, i: number) => (
                <li key={i} className="text-sm text-gray-500 flex items-start gap-2"><span className="text-red-400 mt-0.5">•</span>{r}</li>
              ))}
            </ul>
          </div>
        )}
        ...
        {p.founderDecision && (
          <div className="bg-gray-50 rounded-lg p-3 mb-4">
            <p className="text-xs text-gray-400 mb-0.5">Founder Decision</p>
        ...
        {showActions && (
          <div className="flex gap-3 pt-4 border-t border-gray-100">
            <form action={approveProposal}>
              ...
              <button type="submit" className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-emerald-100 transition">✅ Approve</button>
            </form>
            <form action={rejectProposal}>
              <input type="hidden" name="proposalId" value={p.id} />
              <button type="submit" className="bg-red-50 text-red-600 border border-red-200 px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-red-100 transition">❌ Reject</button>
            </form>
          </div>
        )}
