      ...
      <main className="flex-1 md:ml-60 pt-16 md:pt-0 p-4 md:p-8 pb-24 md:pb-8 max-w-7xl">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/transactions/new" className="bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-800 transition">+ {t("newTx", locale)}</Link>
          </div>
        </div>
        {txs.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
            <p className="text-gray-400 mb-4">{t("noTransactions", locale)}</p>
            <Link href="/transactions/new" className="bg-gray-900 text-white px-6 py-3 rounded-lg font-medium">{t("recordFirst", locale)}</Link>
          </div>
        ) : (
