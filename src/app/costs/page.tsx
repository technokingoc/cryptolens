    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 md:ml-64 pt-16 md:pt-20 p-4 md:p-8 pb-24 md:pb-8 max-w-5xl">
        ...
        <form action={addCostItem} className="bg-white border border-gray-200 rounded-xl p-5 mb-6 space-y-3">
          <h2 className="font-semibold text-gray-700 text-sm mb-2">{t("addCostItem", locale)}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <select name="frequency" required className={inputCls}><option value="monthly">{t("monthly", locale)}</option><option value="annual">{t("annual", locale)}</option><option value="one-time">{t("oneTime", locale)}</option></select>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input name="category" placeholder={t("category", locale)} className={inputCls} />
          ...
          <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">{t("addCost", locale)}</button>
        </form>
